...
import { bnbRuntime } from './bnb-app.js';
import { startBnbSchedulers } from './bnb-schedulers.js';

const schedulerController = startBnbSchedulers(bnbRuntime);
...
async function gracefulShutdown(signal: NodeJS.Signals): Promise<void> {
  ...
  shutdownPromise = (async () => {
    ...
    const shutdownTimeoutMs = positiveTimeout(process.env.SHUTDOWN_TIMEOUT_MS, 15_000);

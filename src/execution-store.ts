...
export class ExecutionStore {
  ...
  private ensureExecutionColumns(): void {
    const columns = this.database.prepare(`PRAGMA table_info(exit_execution_proposals)`).all() as Array<{
      name: string;
    }>;
    if (!columns.some(column => column.name === 'settled_at')) {
      this.database.exec(`ALTER TABLE exit_execution_proposals ADD COLUMN settled_at TEXT`);
    }
    this.database.exec(`
      CREATE UNIQUE INDEX IF NOT EXISTS idx_exit_proposals_one_active_per_position
